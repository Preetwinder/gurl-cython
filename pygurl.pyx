...
cdef class URL:
        cdef cppgurl.GURL *_thisptr
        def __cinit__(self, url):
                self._thisptr = new cppgurl.GURL(url)
        ...
        #cpdef URL Resolve(self, url):
        #        #cdef cppgurl.GURL G = self._thisptr.Resolve(url)
        #        pygurlObj = PyGURLOBJ()
        #        pygurlObj.thisobj = self._thisptr.Resolve(url)
        #        pygurlObj.thisptr = &(pygurlObj.thisobj)
        #        return URL(pygurl = pygurlObj)
